...
const INTERNAL_ROUTES = [
  "/login","/dashboard","/edit-agent","/incidents","/cases","/responder",
];
...
const AppLayout = ({ children }) => {
  ...
  const hideNavbar = INTERNAL_ROUTES.some(p => location.pathname.startsWith(p));
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-6 bg-white text-center">
      <div className="max-w-md">
        ...
        <h3 className="text-2xl font-black uppercase tracking-tighter text-slate-900 mb-4">Route Terminated</h3>
    ...
    <Router>
      <AppLayout>
        <Routes>
          ...
          <Route path="/dashboard/service-admin"
            element={<ProtectedRoute role="serviceadmin"><ServiceAdminDashboard /></ProtectedRoute>} />
          <Route path="/dashboard/agency"
            element={<ProtectedRoute role="agency"><AgencyDashboard /></ProtectedRoute>} />
          <Route path="/edit-agent/:id"
            element={<ProtectedRoute role="agency"><EditAgentPage /></ProtectedRoute>} />
          <Route path="/incidents"
            element={<ProtectedRoute role="agency"><IncidentsPage /></ProtectedRoute>} />
          <Route path="/incidents/:id"
            element={<ProtectedRoute role="agency"><IncidentDetailPage /></ProtectedRoute>} />
          <Route path="/dashboard/responder"
            element={<ProtectedRoute role="responder"><ResponderDashboard /></ProtectedRoute>} />
          <Route path="/responder/incidents"
            element={<ProtectedRoute role="responder"><ResponderIncidentsPage /></ProtectedRoute>}>
            <Route path=":id" element={<ResponderIncidentDetail />} />
          </Route>
          <Route path="/cases/:id"
            element={<ProtectedRoute role="responder"><CaseDetailPage /></ProtectedRoute>} />
