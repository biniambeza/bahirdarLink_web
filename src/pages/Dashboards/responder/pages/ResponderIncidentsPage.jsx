...
const ResponderIncidentsPage = () => {
  ...
  const [isServiceMode, setIsServiceMode] = useState(false);
  const [agencyInfo, setAgencyInfo] = useState(null);
  const [isMerging, setIsMerging] = useState(false); // New state for merge loading
  ...
  const handleMerge = async () => {
    if (selectedForMerge.length < 2) {
      alert("Please select at least two incidents to merge.");
      return;
    }
    ...
    try {
      ...
      // We take the first ID as the 'main' and the rest as 'mergeIds'
      const [mainId, ...mergeIds] = selectedForMerge;

      // Note: Ensure your controller 'mergeEmergencies' destructures
      // these keys: mainId and mergeIds
      const payload = {
        mainId: mainId,
        mergeIds: mergeIds,
      };

      await axios.post(`${API_BASE}/api/emerged/merge`, payload, { headers });

      // Success cleanup
      setSelectedForMerge([]);
      await fetchData();
      alert("Incidents consolidated successfully.");
    } catch (err) {
      console.error("Merge failed:", err);

      // Better error messaging based on your backend 'throw new Error' messages
      const errorMessage =
        err.response?.data?.message ||
        err.response?.data ||
        "Check if items are in the same Kebele and Category.";
      ...
      <header className="bg-white/80 backdrop-blur-md border-b border-slate-200 sticky top-0 z-50">
        <div className="max-w-[1600px] mx-auto px-6 h-16 flex items-center justify-between gap-8">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="h-8 w-[1px] bg-slate-200 mx-2" />
            <div
              className={`w-8 h-8 rounded-full border-2 border-white shadow-sm flex items-center justify-center text-white text-[10px] font-bold bg-gradient-tr ${isServiceMode ? "from-emerald-600 to-teal-600" : "from-blue-600 to-indigo-600"}`}
            >
              RP
            </div>
          </div>
            ...
            <div className="bg-slate-900 text-white p-2 pl-4 rounded-xl shadow-2xl flex items-center gap-6 border border-white/10">
              ...
              <div className="flex gap-1">
                <button
                  onClick={() => setSelectedForMerge([])}
                  disabled={isMerging}
                  className="px-3 py-2 text-xs font-bold hover:bg-white/10 rounded-lg transition-colors disabled:opacity-50"
                >
                  Dismiss
                </button>
                <button
                  ...
                  className={`px-4 py-2 rounded-lg text-xs font-black shadow-lg shadow-blue-900/40 flex items-center gap-2 ${isServiceMode ? "bg-emerald-600 hover:bg-emerald-500" : "bg-blue-600 hover:bg-blue-500"} disabled:opacity-50`}
                >
                  {isMerging ? (
                    <>
