...
/**
 * Helper to handle localization (English)
 */
...
const ResponderCasesPage = () => {
  const [cases, setCases] = useState([]); // Default to empty array
  const [loading, setLoading] = useState(true);
  const [showForm, setShowForm] = useState(false);
  const [searchTerm, setSearchTerm] = useState("");
  const [selectedImage, setSelectedImage] = useState(null);
  ...
  const fetchCases = useCallback(async () => {
    try {
      ...
      // FIX: Ensure we are setting an array. If backend returns { data: [...] }, use that.
      const data = res.data?.data || res.data;
      setCases(Array.isArray(data) ? data : []);
    } catch (err) {
      console.error("Fetch error:", err);
      setCases([]); // Reset to array on error
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  // FIX: Added safe access (?.) and localization helpers
  const filteredCases = (cases || []).filter((c) => {
    const searchStr = searchTerm.toLowerCase();
    const fullName = renderEnglish(c.fullName).toLowerCase();
    const locationName = renderEnglish(c.lastSeenLocation).toLowerCase();
  ...
  return (
    ...
    >
      <div className="relative h-64 bg-slate-100 overflow-hidden">
        ...
        <div className="absolute top-4 left-4 flex flex-col gap-2">
          <span className="px-3 py-1 bg-white/90 backdrop-blur-md text-blue-600 text-[9px] font-black uppercase tracking-widest rounded-lg border border-slate-200 shadow-sm">
            {renderEnglish(c.caseType?.name || c.caseType) || "Standard Case"}
          </span>
          {c.isDangerous && (
            <span className="px-3 py-1 bg-red-600 text-white text-[9px] font-black uppercase flex items-center gap-1 rounded-lg shadow-lg">
              <AlertTriangle size={10} /> High Risk
        ...
        <div className="grid grid-cols-2 gap-4 border-t border-slate-50 pt-6">
          <div className="flex gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs font-black text-slate-900 truncate">
                {renderEnglish(c.lastSeenLocation)}
              </p>
            </div>
          </div>
...
const ImageViewer = ({ src, onClose }) => (
  ...
  >
    <div className="relative">
      ...
      <button
        onClick={onClose}
        className="absolute -top-12 -right-12 text-white p-2 hover:bg-white/10 rounded-full transition-all"
      >
        <X size={32} />
      </button>
    </div>
