...
/* ═══════════════════════════════════════════════════════
   MAIN — Service-based reports, upgraded to advanced UI & logic
═══════════════════════════════════════════════════════ */
const ReportsPage = () => {
    ...
    (async () => {
      ...
      try {
        ...
        const [reportsRes, kebelesRes] = await Promise.all([
          ...
          axios.get(KEBELES_URL, { headers }).catch(() => ({ data: [] })),
        ]);

        if (reportsRes.data.success) {
          setReports(reportsRes.data.data || []);
        } else {
          throw new Error(reportsRes.data.error || "Failed to fetch reports");
        }

        const kebeleList =
          kebelesRes.data?.data ||
          kebelesRes.data?.kebeles ||
          (Array.isArray(kebelesRes.data) ? kebelesRes.data : []);
  ...
  /* ── Kebele dropdown options — full list from API ── */
  ...
  const stats = useMemo(
    () => ({
      ...
      escalated: reports.filter(
        (r) => String(r.status || "").toLowerCase() === "escalated",
      ).length,
      pending: reports.filter((r) =>
        ["pending", "reported"].includes(String(r.status || "").toLowerCase()),
      ).length,
  ...
  const matchesAgency = (report, agencyId) => {
    ...
    const typeStr = String(
      renderEnglish(
        report.serviceType || report.emergencyType || report.agencyType,
      ),
    ).toLowerCase();
  ...
  const filteredReports = useMemo(() => {
    ...
    let list = reports
      ...
      .filter((r) => {
        if (!q) return true;
        return [
          renderEnglish(r.serviceType?.name || r.serviceType),
          renderEnglish(r.serviceCategory?.name || r.serviceCategory),
          r.reporterName,
          r.kebele,
          r.subdivision,
          r.street,
    ...
    return [...list].sort((a, b) => {
      ...
      if (sortCol === "status") {
        const av = STATUS_ORDER[String(a.status || "").toLowerCase()] ?? 99;
        const bv = STATUS_ORDER[String(b.status || "").toLowerCase()] ?? 99;
        return sortDir === "asc" ? av - bv : bv - av;
      }
      const getVal = {
        type: (r) => renderEnglish(r.serviceType?.name || r.serviceType),
                  ...
                  <tbody>
                    {filteredReports.map((report) => {
                      const typeStr = renderEnglish(
                        report.serviceType?.name || report.serviceType,
                      );
                      const categoryStr = renderEnglish(
                        report.serviceCategory?.name || report.serviceCategory,
                      );
