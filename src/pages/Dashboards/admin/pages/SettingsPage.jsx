  ...
  s.textContent = `
    ...
    .shimmer-btn {
      background: linear-gradient(90deg,
        #2563eb 0%, #3b82f6 40%, #60a5fa 50%, #3b82f6 60%, #1d4ed8 100%
      );
...
/* ── section card ── */
const SectionCard = ({ title, subtitle, icon, accentClass, inputPlaceholder, inputValue, onInputChange, onAdd, loading, list, delay }) => (
  <div
    ...
    <div className="p-8">
      ...
      <div className="flex items-start justify-between mb-8">
        <div className="flex items-center gap-4">
          <div className={`section-icon-ring w-12 h-12 rounded-2xl flex items-center justify-center text-white text-xl shadow-lg ${accentClass}`}>
            {icon}
          </div>
          <div>
            <h2 className="text-[1.15rem] font-bold text-slate-900 leading-tight" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
              {title}
            </h2>
      ...
      <div className="min-h-[56px]">
        {list.length > 0 ? (
          <div className="flex flex-wrap gap-2.5">
            {list.map((item, i) => (
              <TagChip key={item.id || i} name={item.name} index={i} />
            ))}
          </div>
        ) : (
          <div className="flex items-center gap-3 py-4 px-5 bg-slate-50 rounded-2xl border border-dashed border-slate-200">
            <span className="text-2xl opacity-30">🏷️</span>
            <p className="text-[13px] text-slate-400 font-medium">No types added yet — be the first!</p>
          </div>
        )}
      </div>
    </div>
...
const SettingsPage = () => {
  ...
  const fetchAgencyTypes = async () => {
    try {
      ...
      if (!token) return;
      const res = await axios.get("http://localhost:5000/api/agencyType/my-agents", {
      ...
      <div className="relative max-w-3xl mx-auto px-6 py-12">
        ...
        <div className="fade-up mb-12 flex items-end justify-between">
          <div>
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-[10px] font-bold tracking-[0.22em] uppercase text-blue-500">Admin Panel</span>
            </div>
            <h1
            ...
            >
              ...
              <span className="text-transparent bg-clip-text"
                style={{ backgroundImage: "linear-gradient(90deg,#2563eb,#38bdf8)" }}>
                Settings
              </span>
            </h1>
          ...
          <Dots className="text-blue-400 opacity-60 hidden md:block" />
        </div>
        ...
        <div className="space-y-6">
          ...
          <SectionCard
            ...
            onAdd={() => handlePost("agencyType", agencyTypeInput, setAgencyTypeInput, setAgencyLoading, fetchAgencyTypes)}
          ...
          <SectionCard
            ...
            onAdd={() => handlePost("emergencyType", emergencyTypeInput, setEmergencyTypeInput, setEmergencyLoading, fetchEmergencyTypes)}
