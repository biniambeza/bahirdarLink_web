...
const AddAgentPanel = ({ closePanel, refreshAgents }) => {
  ...
  const handleAgencyTypeChange = (e) => {
    setForm({ ...form, agencyTypeId: e.target.value });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // The backend controller will automatically add 'createdBy' using the token.
      await axios.post(
        "http://localhost:5000/api/agency",
        {
          ...form,
          agencyTypeId: Number(form.agencyTypeId), // Ensure it's a number for Sequelize
        },
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      ...
      refreshAgents(); // This will now fetch only the agents created by THIS admin
      closePanel();
    } catch (err) {
      setError(
        err.response?.data?.message || err.response?.data?.error || err.message,
