...
import {
  Flame, Droplets, Skull, Ambulance, Radio, Search,
  ChevronRight, TrendingUp, TrendingDown, Shield, Users,
  Activity, RefreshCw, AlertCircle, CheckCircle2, X,
} from "lucide-react";
import {
...
const DEFAULT_PALETTE = [
  "#3B82F6","#6366F1","#EF4444","#06B6D4","#10B981",
  "#F59E0B","#F97316","#8B5CF6","#EC4899","#14B8A6","#84CC16","#DC2626",
];
const FORCED_COLORS = {
  fire:"#F97316", crime:"#6366F1", medical:"#EF4444",
...
const TEAM_COLORS = ["#3B82F6","#EF4444","#6366F1","#06B6D4","#10B981","#F59E0B","#F97316","#8B5CF6"];

const STATUS_META = {
  reported:    { label:"Reported",    color:"#D97706", bg:"#FFFBEB", border:"#FDE68A" },
  ...
  pending:     { label:"Reported",    color:"#D97706", bg:"#FFFBEB", border:"#FDE68A" },
  active:      { label:"Reported",    color:"#D97706", bg:"#FFFBEB", border:"#FDE68A" },
  responding:  { label:"In Progress", color:"#2563EB", bg:"#EFF6FF", border:"#BFDBFE" },
};
...
const ChartTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div style={{
      ...
      <p style={{ color:"#64748B", fontWeight:600, marginBottom:4, fontSize:11 }}>{label}</p>
      {payload.map((p, i) => (
        <p key={i} style={{ color:"#0F172A", fontWeight:700 }}>
          {p.name || "Count"}: <span style={{ color:p.color||"#3B82F6" }}>{p.value}</span>
        </p>
      ))}
    </div>
  );
...
const Sk = ({ h = 16, w = "100%", r = 8 }) => (
  <div style={{ height:h, width:w, background:"#F1F5F9", borderRadius:r, animation:"shimmer 1.6s ease infinite" }} />
);
...
const ErrorBanner = ({ msg, onRetry }) => (
  <div style={{
    ...
    background:"#FEF2F2", border:"1px solid #FECACA", borderRadius:10, marginBottom:14,
  }}>
    <AlertCircle size={14} color="#EF4444" />
    <span style={{ fontSize:12, color:"#B91C1C", flex:1, fontFamily:"'Plus Jakarta Sans',sans-serif" }}>{msg}</span>
  ...
  return (
    <>
      <div onClick={onClose} style={{
        position:"fixed", inset:0, background:"rgba(2,8,23,0.5)",
      ...
      <div style={{
        ...
        width:"50%", minWidth:480, background:"#fff", zIndex:100,
        ...
        <button onClick={onClose} style={{
          position:"sticky", top:16, left:16, zIndex:10, alignSelf:"flex-start",
          margin:"16px 0 0 16px", display:"flex", alignItems:"center", gap:6,
        ...
        {incident && <div style={{ flex:1 }}><IncidentDetailPage incidentProp={incident} panelMode /></div>}
      </div>
    </>
  );
};
...
const DashboardMain = () => {
  ...
  const agencyRows = useMemo(() =>
    agencies.slice(0,8).map((a,i) => {
      ...
      return { initials:getInitials(name), name, role, active:i%4!==2, color:TEAM_COLORS[i%TEAM_COLORS.length] };
    }), [agencies]);
  ...
  return (
    <>
      ...
      <div style={{
        fontFamily: F, background:"#F8FAFC", minHeight:"100vh",
        padding:"24px 28px", color:"#0F172A",
      }}>
        ...
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:24 }}>
          <div>
            <div style={{
              ...
              <span style={{ fontSize:11, fontWeight:600, color:"#64748B", fontFamily:FM, letterSpacing:".06em" }}>
                LIVE DASHBOARD
              </span>
            </div>
            <h1 style={{ fontSize:22, fontWeight:800, color:"#0F172A", letterSpacing:"-.03em", lineHeight:1.2 }}>
              Emergency Control Center
            </h1>
            <p style={{ fontSize:11, color:"#94A3B8", marginTop:4, fontFamily:FM }}>
              {lastSync ? `Synced at ${format(lastSync,"HH:mm:ss")}` : "Loading…"}
            </p>
          </div>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                ...
                {categories.slice(0,8).map((cat,i) => {
                  ...
                  return (
                    <div key={cat.id||cat._id||i} className="cat-pill" style={{
                      ...
                      fontSize:10, color, fontWeight:700, fontFamily:FM,
                      transition:"all .15s", cursor:"default", letterSpacing:".04em",
                    }}>
            ...
            <button className="rfrsh-btn" onClick={fetchAll} style={{
              ...
              fontSize:11, color:"#475569", cursor:"pointer", fontFamily:FM,
              fontWeight:600, transition:"all .15s", letterSpacing:".05em",
              boxShadow:"0 1px 3px rgba(0,0,0,0.05)",
            }}>
              <RefreshCw size={12} style={{ animation:isLoadingAny?"spin 1s linear infinite":"none" }} />
              REFRESH
            </button>
          </div>
        </div>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:14, marginBottom:20 }}>
          {[
            { label:"TOTAL INCIDENTS", value:stats.total, sub:"in database", trend:`${stats.total} total`, up:false, accent:"#3B82F6", icon:Activity, delay:0, load:loading.emergencies },
            { label:"RESOLVED", value:stats.resolved, sub:"closed", trend:`${stats.resRate}% rate`, up:false, accent:"#10B981", icon:CheckCircle2, delay:60, load:loading.emergencies },
            { label:"RESPONDER TEAMS", value:teams.length, sub:"registered", trend:`${teamRows.filter(t=>t.deployed>0).length} active`, up:false, accent:"#6366F1", icon:Shield, delay:120, load:loading.teams },
            { label:"AGENCIES", value:agencies.length, sub:"registered", trend:`${agencyRows.filter(a=>a.active).length} on duty`, up:false, accent:"#F97316", icon:Users, delay:180, load:loading.agencies },
          ].map(({ label, value, sub, trend, up, accent, icon:Icon, delay, load }) => (
            <div key={label} style={{
              ...
              <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:14 }}>
                <span style={{ fontSize:10, fontWeight:700, letterSpacing:".12em", color:"#94A3B8", fontFamily:FM }}>
                  {label}
                </span>
                <div style={{
                  width:34, height:34, borderRadius:9, background:`${accent}12`,
              ...
              <div style={{
                ...
                letterSpacing:"-.04em", lineHeight:1, marginBottom:10,
              }}>
                {load ? <span style={{ fontSize:18, color:"#CBD5E1" }}>—</span> : value}
              </div>
              <div style={{ display:"flex", alignItems:"center", gap:6, fontSize:10, fontFamily:FM }}>
                <span style={{
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1.7fr 1fr", gap:16, marginBottom:16 }}>
          ...
          <div style={{
            ...
            <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", marginBottom:20 }}>
              <div>
                <div style={{ display:"flex", alignItems:"center", gap:7, marginBottom:3 }}>
                  ...
                  <span style={{ fontSize:12, fontWeight:700, color:"#0F172A", letterSpacing:".04em" }}>
                    INCIDENT TIMELINE
                  </span>
                </div>
                <p style={{ fontSize:11, color:"#94A3B8", marginLeft:10, fontFamily:FM }}>
              ...
              <div style={{ display:"flex", background:"#F1F5F9", borderRadius:8, padding:3, gap:2 }}>
                {["daily","weekly","monthly"].map((r) => (
                  <button key={r} className="range-btn" onClick={()=>setRange(r)} style={{
                    ...
                    fontSize:10, fontWeight:700, letterSpacing:".08em", fontFamily:FM,
                    textTransform:"uppercase",
                    background: range===r ? "#3B82F6" : "transparent",
                    color: range===r ? "#fff" : "#64748B",
                    boxShadow: range===r ? "0 1px 4px rgba(59,130,246,.3)" : "none",
                    transition:"all .15s",
                  }}>
                    {r}
                  </button>
            ...
            {loading.emergencies ? (
              <div style={{ display:"flex", alignItems:"flex-end", gap:4, height:200, paddingBottom:8 }}>
              ...
              <ResponsiveContainer width="100%" height={200}>
                <BarChart data={chartData} barSize={range==="monthly"?18:range==="weekly"?36:20}>
                  <CartesianGrid strokeDasharray="2 4" stroke="#F1F5F9" vertical={false} />
                  <XAxis dataKey="label" tick={{ fontSize:11, fill:"#94A3B8", fontFamily:FM }} axisLine={false} tickLine={false} />
                  <YAxis tick={{ fontSize:11, fill:"#94A3B8", fontFamily:FM }} axisLine={false} tickLine={false} width={28} />
                  <Tooltip content={<ChartTooltip />} cursor={{ fill:"rgba(59,130,246,0.04)", radius:6 }} />
                  <Bar dataKey="count" name="Incidents" radius={[5,5,0,0]}>
                    {chartData.map((_,i)=>(
                      <Cell key={i} fill={i===chartData.length-1?"#F97316":"#3B82F6"} opacity={i===chartData.length-1?1:0.8} />
                    ))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
          ...
          <div style={{
            ...
            <p style={{ fontSize:11, color:"#94A3B8", marginLeft:10, marginBottom:16, fontFamily:FM }}>
            ...
            {loading.emergencies||loading.categories ? (
              <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
                <Sk h={140} r={12} />
                {[1,2,3,4].map(i=><Sk key={i} h={12} />)}
              </div>
            ) : catPie.length===0 ? (
              <div style={{ textAlign:"center", color:"#94A3B8", paddingTop:60, fontSize:12, fontFamily:F }}>
                No data yet.
              </div>
            ) : (
              <>
                <ResponsiveContainer width="100%" height={160}>
                  <PieChart>
                    <Pie data={catPie} cx="50%" cy="50%" innerRadius={46} outerRadius={72}
                    ...
                    return (
                      <div key={i} style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
                        <div style={{ display:"flex", alignItems:"center", gap:7 }}>
                          ...
                          <div style={{
                            width:18, height:18, borderRadius:5, background:c.color+"18",
                            display:"flex", alignItems:"center", justifyContent:"center",
                          }}>
                        ...
                        <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                          <div style={{ width:40, height:4, borderRadius:3, background:"#F1F5F9", overflow:"hidden" }}>
                            <div style={{ width:`${pct}%`, height:"100%", background:c.color, borderRadius:3 }} />
                          </div>
                          <span style={{ fontSize:12, fontWeight:700, color:c.color, fontFamily:FM, minWidth:16, textAlign:"right" }}>
                            {c.value}
                          </span>
                        </div>
                      </div>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1.55fr 1fr 0.85fr", gap:16 }}>
          ...
          <div style={{
              ...
              <div style={{ position:"relative" }}>
                ...
                <input className="s-input" value={search} onChange={(e)=>setSearch(e.target.value)}
                  placeholder="Search…" style={{
                    ...
                    fontSize:12, color:"#0F172A", fontFamily:FM, width:160, transition:"all .2s",
                  }}
                />
              </div>
            </div>
            ...
            {loading.emergencies ? (
              <div style={{ display:"flex", flexDirection:"column", gap:7 }}>
                {[1,2,3,4,5,6].map(i=><Sk key={i} h={54} r={10} />)}
              </div>
            ) : filtered.length===0 ? (
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
                {filtered.map((r) => {
                  ...
                  return (
                    <div key={r.id||r._id} className="inc-row" onClick={()=>setSelected(r)} style={{
                      display:"flex", alignItems:"center", gap:11, padding:"11px 13px",
                      background:"#FAFBFF", border:"1px solid #E8EFFE", borderRadius:11,
                      ...
                      <div style={{ flex:1, minWidth:0 }}>
                        <div style={{ display:"flex", alignItems:"center", gap:6, marginBottom:2 }}>
                          ...
                          <div style={{ fontSize:9, padding:"2px 6px", borderRadius:4, background:color+"14", color, fontWeight:700, fontFamily:FM, flexShrink:0 }}>
                            {catLabel}
                          </div>
                        </div>
                        <div style={{ fontSize:10.5, color:"#94A3B8", fontFamily:FM }}>{locEn}</div>
                      </div>
                      <div style={{ fontSize:10, color:"#CBD5E1", fontFamily:FM, whiteSpace:"nowrap", flexShrink:0 }}>
                        {formatDistanceToNow(new Date(r.createdAt),{addSuffix:true})}
                      </div>
                      <span style={{
                        fontSize:9, fontWeight:700, padding:"3px 8px", borderRadius:5,
                        background:st.bg, color:st.color, border:`1px solid ${st.border}`,
                        letterSpacing:".06em", fontFamily:FM, flexShrink:0,
                      }}>
          ...
          <div style={{
            ...
            <p style={{ fontSize:11, color:"#94A3B8", marginLeft:10, marginBottom:18, fontFamily:FM }}>
            ...
            {loading.teams ? (
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
                {teamRows.map((t,i) => {
                  ...
                  return (
                    <div key={i} className="team-row" style={{ padding:"9px 10px", borderRadius:10, transition:"background .15s" }}>
                      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:7 }}>
                        <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                          <div style={{ width:24, height:24, borderRadius:7, background:t.color+"14", display:"flex", alignItems:"center", justifyContent:"center" }}>
                            <Shield size={11} color={t.color} strokeWidth={2.5} />
                          </div>
                          <span style={{ fontSize:12, fontWeight:700, color:"#1E293B", whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis", maxWidth:110 }}>
                            {t.name}
                          </span>
                        </div>
                        <span style={{ fontSize:10, color:"#94A3B8", fontFamily:FM }}>{t.deployed}/{t.total}</span>
                      </div>
                      <div style={{ height:5, background:"#F1F5F9", borderRadius:3, overflow:"hidden" }}>
                        <div style={{ width:`${pct}%`, height:"100%", background:t.color, borderRadius:3, transition:"width .8s ease" }} />
                      </div>
                      <div style={{ display:"flex", justifyContent:"space-between", marginTop:4 }}>
                        <span style={{ fontSize:9, color:loadColor, fontFamily:FM, fontWeight:700 }}>{load}</span>
                        <span style={{ fontSize:9, fontWeight:700, color:t.color, fontFamily:FM }}>{pct}%</span>
                      </div>
                    </div>
                  );
                })}
          ...
          <div style={{
            ...
            <div style={{ display:"flex", alignItems:"center", gap:7, marginBottom:4 }}>
              ...
              <span style={{ fontSize:12, fontWeight:700, color:"#0F172A", letterSpacing:".04em" }}>AGENCIES</span>
            </div>
            <p style={{ fontSize:11, color:"#94A3B8", marginLeft:10, marginBottom:16, fontFamily:FM }}>
            ...
            {loading.agencies ? (
              <div style={{ display:"flex", flexDirection:"column", gap:7 }}>
                {[1,2,3,4,5].map(i=><Sk key={i} h={42} r={10} />)}
              </div>
            ) : agencyRows.length===0 ? (
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:6, flex:1 }}>
                {agencyRows.map((a,i) => (
                  <div key={i} className="agency-row" style={{
                    ...
                    <div style={{
                      width:30, height:30, borderRadius:"50%", background:`${a.color}14`,
                      display:"flex", alignItems:"center", justifyContent:"center",
                    ...
                    <div style={{ flex:1, minWidth:0 }}>
                      ...
                      <div style={{ fontSize:10, color:"#94A3B8", fontFamily:FM, marginTop:1 }}>{a.role}</div>
                    </div>
                    <div style={{
                      width:7, height:7, borderRadius:"50%", flexShrink:0,
                      background:a.active?"#22C55E":"#F59E0B",
                      boxShadow:a.active?"0 0 0 3px rgba(34,197,94,0.18)":"0 0 0 3px rgba(245,158,11,0.18)",
                    }} />
                  </div>
                ))}
              </div>
            ...
            {/* Summary */}
            <div style={{
              marginTop:14, padding:"14px 15px", background:"#F8FAFC",
              borderRadius:12, border:"1px solid #E2E8F0",
            }}>
              <div style={{ fontSize:10, color:"#64748B", fontFamily:FM, letterSpacing:".1em", marginBottom:10, fontWeight:700 }}>
                NETWORK SUMMARY
              </div>
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:7 }}>
                {[
                  ...
                  <div key={i} style={{ background:"#fff", borderRadius:8, padding:"8px 10px", border:"1px solid #E2E8F0" }}>
                    <div style={{ fontSize:20, fontWeight:800, color:s.color, letterSpacing:"-.03em", lineHeight:1 }}>{s.val}</div>
                    <div style={{ fontSize:10, color:"#94A3B8", fontFamily:FM, marginTop:3 }}>{s.lbl}</div>
                  </div>
                ))}
              </div>
            </div>
